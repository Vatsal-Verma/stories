...
const StoryMap = () => {
  ...
  const handleCountrySelect = country => {
    if (selectedCountry === country) {
      ...
      if (valid > 0) {
        mapInstanceRef.current?.setView([totalLat / valid, totalLng / valid], 5);
      }
    }
  };
          ...
          {showFilters && (
            <div className="tmap-filter-panel">
              <div className="tmap-filter-group">
                <label htmlFor="tmap-industry-filter" className="tmap-filter-label">
                  Filter by Industry:
                </label>
                <select
              ...
              return (
                ...
                >
                  <h3 className="tmap-country-name">
                    {country}{' '}
                    <span className="tmap-country-count">({visible.length})</span>
                  </h3>

                  {isSelected && (
                    <ul className="tmap-story-list">
                      {visible.map(story => (
                        <li key={story.id || story.slug} className="tmap-story-item">
                           <a
